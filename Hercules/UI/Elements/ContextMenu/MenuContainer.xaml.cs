...
using Hercules.UI.Chat;
using Hercules.UI.Elements.Crosshair;
using Hercules.UI.Elements.FPS;
using Hercules.UI.Elements.Overlay;
using Hercules.UI.Elements.Settings.Pages;
...
namespace Hercules.UI.Elements.ContextMenu
{
    ...
    public partial class MenuContainer
    {
        ...
        private void MemoryTimer_Tick(object? sender, EventArgs e)
        {
            try
            {
                var herculesProcess = Process.GetProcessesByName("Hercules").FirstOrDefault();
                long herculesMemory = herculesProcess?.WorkingSet64 ?? 0;
        ...
        public void ActivityWatcher_OnGameJoin(object? sender, EventArgs e)
        {
            ...
            if (App.Settings.Prop.IngameChatDiscord)
            {
                Application.Current.Dispatcher.Invoke(() =>
                {
                    if (App.Current.Resources["DiscordChatOverlayWindow"]
                        is Hercules.UI.Elements.Overlay.DiscordChatOverlayWindow existing)
                    {
                    ...
                    var discordOverlay = new Hercules.UI.Elements.Overlay.DiscordChatOverlayWindow();
                    discordOverlay.Show();
            ...
            if (App.Settings.Prop.OverlaysEnabled)
            {
                Application.Current.Dispatcher.Invoke(() =>
                {
                    if (App.Current.Resources["OverlayWindow"] is Hercules.UI.Elements.Overlay.OverlayWindow existing)
                    {
                        if (existing.IsLoaded)
                        {
                            return;
                    ...
                    var overlay = new Hercules.UI.Elements.Overlay.OverlayWindow();
                    overlay.Show();
        ...
        public void ActivityWatcher_OnGameLeave(object? sender, EventArgs e)
        {
            Application.Current.Dispatcher.Invoke(() =>
            {
                ...
                if (App.Current.Resources["OverlayWindow"] is Hercules.UI.Elements.Overlay.OverlayWindow overlay)
                {
                    overlay.Close();
                    App.Current.Resources.Remove("OverlayWindow");
                }

                if (App.Current.Resources["DiscordChatOverlayWindow"] is Hercules.UI.Elements.Overlay.DiscordChatOverlayWindow discordOverlay)
                {
