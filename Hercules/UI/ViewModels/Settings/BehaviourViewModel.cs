...
using Hercules.AppData;
using Hercules.RobloxInterfaces;
using static HerculesRobloxSettingsManager;


namespace Hercules.UI.ViewModels.Settings
{
    public class BehaviourViewModel : NotifyPropertyChangedViewModel
    {
        ...
        {
            var settings = HerculesRobloxSettingsManager.Load();
        ...
        {
            var settings = new HerculesRobloxSettings
            {
                MemoryCleanerIntervalSeconds =
                    SelectedMemoryCleanerInterval?.Seconds ?? 0
            };

            HerculesRobloxSettingsManager.Save(settings);
        }
        ...
        public bool CleanerHercules
        {
            get => CleanerItems.Contains("HerculesLogs");
            set
            {
                if (value && !CleanerItems.Contains("HerculesLogs"))
                {
                    CleanerItems.Add("HerculesLogs");
                    UpdateCleanerItems();
                }
                else if (!value && CleanerItems.Contains("HerculesLogs"))
                {
                    CleanerItems.Remove("HerculesLogs");
                    UpdateCleanerItems();
                }
                OnPropertyChanged(nameof(CleanerHercules));
            }
        }
