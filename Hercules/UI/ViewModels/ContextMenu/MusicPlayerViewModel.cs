    ...
    Task.Run(() =>
    {
        ...
        private void UpdateRpcPresence(bool force = false)
        {
            ...
            if (NowPlaying == null || string.IsNullOrWhiteSpace(NowPlaying.Title))
            {
                ...
                {
                    Details = "Idle",
                    State = "Hercules Music Player",
                    Assets = new DRPC.Assets
                    {
                        LargeImageKey = "hercules_logo",
                        LargeImageText = "Hercules Music Player"
                    }
                };
            }
            else
            {
                ...
                string infoString = $"{(NowPlaying.FileType ?? "FILE").ToUpperInvariant()} • {elapsedStr} / {totalStr} • {(_isPlaying ? "Playing" : "Paused")} in Hercules{loopText}";
